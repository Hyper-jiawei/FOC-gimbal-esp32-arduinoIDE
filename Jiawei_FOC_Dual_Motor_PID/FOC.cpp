...
PIDController current_loop_M0(1.2, 0, 0, 100000, 12.6);
PIDController vel_loop_M1(2, 0, 0, 100000, voltage_power_supply/2);
PIDController angle_loop_M1(2, 0, 0, 100000, 100);
PIDController current_loop_M1(1.2, 0, 0, 100000, 12.6);
//AS5600
...
void M0_setPwm(float Ua, float Ub, float Uc) {
  ...
  ledcWrite(M0_pwmA, uint32_t(dc_a * 255));
  ledcWrite(M0_pwmB, uint32_t(dc_b * 255));
  ledcWrite(M0_pwmC, uint32_t(dc_c * 255));
}
...
void DFOC_Vbus(float power_supply)
{
  ...
  angle_loop_M0 = PIDController(2, 0, 0, 100000, 100);
  current_loop_M0 = PIDController(1.2, 0, 0, 100000, 12.6);
  vel_loop_M1 = PIDController(2, 0, 0, 100000, voltage_power_supply/2);
  angle_loop_M1 = PIDController(2, 0, 0, 100000, 100);
  current_loop_M1 = PIDController(1.2, 0, 0, 100000, 12.6);
  //初始化电流传感器
  CS_M0.init();
  CS_M1.init();
 }
